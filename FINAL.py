...
class FlowerShopUI:
    def __init__(self):
        self.conn = sqlite3.connect("projectfinal.db")
    ...
    def delete_flower_arrangement_window(self):
        # Fetch all flower arrangement details from the database
        self.cur.execute("SELECT * FROM Flower_arrangement")
        arrangement_details = self.cur.fetchall()
    
        if not arrangement_details:
            print("No flower arrangements found in the database.")  
            sg.popup('No flower arrangements found.')
            return
    
        layout = [
            [sg.Text('Select Flower Arrangement to Delete:')],
            [sg.Listbox(values=[f"{arrangement[0]} - {arrangement[1]}" for arrangement in arrangement_details], size=(30, 6), key='-ARRANGEMENT_LIST-')],
            [sg.Button('Delete'), sg.Button('Close')]
        ]
    
        window = sg.Window('Choose Flower Arrangement', layout)
        ...
        while True:
            ...
            if event == sg.WIN_CLOSED or event == 'Close':
                ...
                selected_arrangement = values['-ARRANGEMENT_LIST-'][0].split(' - ')[0]  # Extract ID from selected item
                self.cur.execute("DELETE FROM Flower_arrangement WHERE FID = ?", (selected_arrangement,))
                self.conn.commit()
    ...
    def view_assigned_orders(self):
        ...
        self.cur.execute("""
            ...
            WHERE u.DID = ? 
        """, (self.logged_in_user,))
        assigned_orders_data = self.cur.fetchall()
    
        if not assigned_orders_data:
            sg.popup('No assigned orders found.')
            return
    
        # Convert status from boolean to human-readable form
        display_data = [(order_id, order_date, delivery_date, paid_price, 'Completed' if status else 'Incomplete') for order_id, order_date, delivery_date, paid_price, status in assigned_orders_data]
    
        layout = [
            [sg.Text('Assigned Orders')],
            [sg.Table(values=display_data, headings=['Order ID', 'Order Date', 'Delivery Date', 'Paid Price', 'Status'], key='-ORDERS-', enable_events=True)],
            [sg.Button('Edit'), sg.Button('Close')]
        ]
        ...
        while True:
            ...
            if event == sg.WIN_CLOSED or event == 'Close':
                ...
                if values['-ORDERS-']:  # Ensure a row is selected
                    ...
                    selected_order = assigned_orders_data[selected_order_index]
                    self.edit_order_status(selected_order)
                else:
                    sg.popup('Please select an order to edit.')
        window.close()
    
    def edit_order_status(self, selected_order):
        order_id, _, _, _, current_status = selected_order
    
        new_delivery_date = sg.popup_get_text('Enter new delivery date (YYYY-MM-DD):')
        new_status = sg.popup_get_text('Enter new status (Completed/Incomplete):')
    
        new_status_bool = True if new_status.lower() == 'completed' else False
    
        self.cur.execute("UPDATE Orders SET delivery_date = ? WHERE OrderID = ?", (new_delivery_date, order_id))
        self.cur.execute("UPDATE Updates SET Ustatus = ? WHERE OrderID = ?", (new_status_bool, order_id))
        self.conn.commit()
        sg.popup('Order and status updated successfully!')
    ...
    def admin_login(self):
        while True:
            ...
            if event == sg.WIN_CLOSED or event == 'Cancel':
                ...
                user_id = self.admin_authentication(username, password)
                if user_id:
                    self.logged_in_user = 'admin'
                    self.user_id = user_id  # Admin ID'sini sakla
                    self.admin_main_menu()  # admin_main_menu() fonksiyonunu doğrudan çağırıyorum
                    break
                else:
                    sg.popup('Invalid username or password for admin.')
            elif event == 'Logout':
                self.logged_in_user = None
                break
    ...
    def admin_authentication(self, username, password):
        ...
        if result:
            ...
            self.cur.execute("SELECT AID FROM Admin WHERE AID = ?", (admin_id,))
        ...
        while True:
            ...
            if event == sg.WIN_CLOSED or event == 'Cancel':
                ...
                if user_id:
                    ...
                    sg.popup('Invalid username or password for customer.')
            elif event == 'Logout':
                self.logged_in_user = None
                break
    ...
    def customer_authentication(self, username, password):
        ...
        if result:
            ...
            # Check if the user ID exists in the Customer table
            self.cur.execute("SELECT CID FROM Customer WHERE CID = ?", (customer_id,))
            if self.cur.fetchone():
                return customer_id
        return None
    ...
    def deliverer_login(self):
        while True:
            ...
            if event == sg.WIN_CLOSED or event == 'Cancel':
                ...
                username = values['-USERNAME-']
                password = values['-PASSWORD-']
                deliverer_id = self.deliverer_authentication(username, password)
                if deliverer_id:
                    self.logged_in_user = deliverer_id
                    self.deliverer_main_menu()
                    break
                else:
                    sg.popup('Invalid username or password for deliverer.')
            elif event == 'Logout':
                self.logged_in_user = None
                break
    ...
    def deliverer_authentication(self, username, password):
        ...
        result = self.cur.fetchone()
        if result:
            deliverer_id = result[0]
            # Check if the user ID exists in the Deliverer table
            self.cur.execute("SELECT DID FROM Deliverer WHERE DID = ?", (deliverer_id,))
            if self.cur.fetchone():
                return deliverer_id
        return None
        ...
        while True:
            ...
            if event == sg.WIN_CLOSED or event == 'Close':
                ...
                if values['-TABLE-']:  # Check if any row is selected
                    ...
                    # Inserting into updates table
                    self.cur.execute("INSERT INTO updates (OrderID, DID, Ustatus) VALUES (?, ?, ?)", (self.current_order_id, did, False))
                    self.conn.commit()
